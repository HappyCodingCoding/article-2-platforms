#!/usr/bin/env python3
"""Main script for the qingyun-xhs-title skill: runs both mechanical gates in one pass.
...
with # are ignored. A leading `1. `, `- `, or `xhs: ` is stripped, so a list
pasted straight out of the draft still works.

Reports only the two mechanical gates: length (measure) and compliance
(lint_compliance). 泛人群标签, 三秒闸, and the de-AI rules are all judged by
the agent through reasoning, not by this script - read at Step 7 and applied
per candidate at Step 8, from references/de-ai-writing.md and
references/scoring-and-redlines.md. A FAIL is eliminated - regenerate
replacements and re-run. Exits 1 when anything failed.
"""
...
_PREFIX = re.compile(r'^(?:xhs[:：]\s*|\d+[.、)）]\s*|[-*]\s*)')
...
def check(title):
    """Run both mechanical gates over one title and merge the results."""
...
def render(rows):
    ...
    lines += [
        ...
        "去 AI 味规则、泛人群标签、三秒闸全部是语义层，脚本一条都不查——起草阶段",
        "（Step 7 读 de-ai-writing.md，Step 8 逐条把关）由 AI 内化和判断。",
        "脚本这里只查字符数和合规红线两件事。",
        "红线里的擦边、蹭灾难热点、未证实的负面指控、把主观感受写成客观效果也是语义层——都要人工过。",
        "另外脚本查不出槽位：每条候选所选公式自身的 Slots 填了几个，要自己标。",
    ]
    return "\n".join(lines)
...
def main():
    ap = argparse.ArgumentParser(
        description="qingyun-xhs-title gates: length / compliance")
