...
（页面上没有任何一个 input 的 `accept` 是 `image/*`；按 `accept` 精确挑，
别按描述找，附件那个 input 也收 `.png`/`.jpg`）；用浏览器工具把本地文件写进这个
input（**不要去点它**，点击会打开自动化无法填写的系统原生文件框）。

**文件上传工具本身就会触发 input 的 change 事件**，弹窗只是反应慢，
**不要再手动派发 `change`/`input` 事件**——处理器会跑两遍，图片被插入两次。
等弹窗底部出现「已上传 N 张图片」后，再点「插入图片」。
