...
| `--process-md-only` | option | `yes` / `no` | `no` | `yes` forces `--image-hosting` to `yes` regardless of what it was set to, then stops the workflow after Step 3 — no browser is touched. The processed markdown is copied to `~/Downloads` and its path reported to the user. `no` runs the full workflow through Step 10 as normal. |

## Workflow

### Step 1 — Resolve `--source` and `--title`

Classify `--source` (local path / Notion link / other online link) and state the classification before proceeding. State `--process-md-only` first: when it is `yes`, say that `--image-hosting` will be treated as `yes` regardless of what it was set to, and that the run stops after Step 3 with the processed markdown copied to `~/Downloads` — no browser step runs. Otherwise state `--image-hosting`, and when it is `yes` (the default) say that the article's images will be uploaded to a public host. If `--title` wasn't given, say where the title will come from: the source's first heading for a local file or an online markdown link, or `properties.title` for a Notion page, which Step 2 lifts out separately. State `--driver` as given; Step 4 resolves `auto` (skip this when `--process-md-only` is `yes` — no driver is needed).

If the source is a Notion page and `--process-md-only` is `no`, run Step 4 up to and including its setup gate now, before Step 2 — see the note there. Skip this when `--process-md-only` is `yes`: no browser is opened in that mode.
...
Pass `--image-hosting no` only when `--image-hosting` is `no` **and** `--process-md-only` is `no`. When `--process-md-only` is `yes`, never pass `--image-hosting no` — the script always runs with hosting on, even if the user set `--image-hosting no`.
...
**Why block separation is normalized.** Markdown starts a new block on a *blank* line; a single newline is only a soft wrap. `notion-fetch` returns one block per line with no blank lines at all, and imported that way every consecutive paragraph fuses into one, a paragraph next to a list is swallowed into the list item above it, a list beginning `1.` silently continues the previous list instead of starting its own, and two separate blockquotes fuse into one (or into one paragraph inside it). The script decides per adjacent pair rather than per file, so a source that has blank lines in some places and not others — around its images but not its paragraphs, say — is repaired everywhere it needs to be. A file that is already well-formed has no adjacent block lines to act on and comes through byte-identical. Where two paragraph lines meet, the line above is what decides: ending on sentence-final punctuation means two blocks, breaking mid-sentence means one soft-wrapped block, counted in `soft_wrapped_lines` — except a line that is only a bold span, like `**使用**` or `**安装**`, which reads as a heading-like label and always starts (or ends) a block on its own, regardless of punctuation. Two quote lines follow the same logic, with one exception: a bare `>` on either side is markdown's own idiom for a paragraph break *inside* one blockquote, and keeps them glued as a single block.
...
**If `--process-md-only` is `yes`, stop here — do not run Step 4 onward.** Copy the manifest's `processed_markdown` to `~/Downloads/<title>.md` (sanitize `title` into a safe filename; if a file of that name already exists there, ask before overwriting it rather than silently replacing it). Report the destination path to the user and end the run. Nothing else in this skill runs in this mode: no browser is opened, and no draft is created on Zhihu.
...
With `--process-md-only yes`, the output is different: no draft is created, and the report is just the path of the processed markdown file copied to `~/Downloads`.
